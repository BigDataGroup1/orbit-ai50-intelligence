...
#     local_path = Path('/home/airflow/data/forbes_ai50_seed.json')
#     local_path.parent.mkdir(parents=True, exist_ok=True)
...
#             if file_path.is_file() and session_name in str(file_path):
#                 try:
#                     relative = file_path.relative_to(Path('/home/airflow/data'))
#                     blob_name = f"data/{relative}"
...
"""
DAG 2: AI50 Daily Refresh - Using Bulletproof Scraper
Uses scraper_robust.py's 5-layer fallback system for dynamic pages
Schedule: Daily at 3 AM UTC

DEPLOYMENT:
1. Make sure DAG 1 ran successfully first!
2. Upload this file: gsutil cp ai50_daily_refresh_robust_dag.py gs://us-central1-orbit-airflow-e-2044600e-bucket/dags/
3. Make sure scraper_robust.py is uploaded to dags/
4. Wait 2-3 minutes
5. Enable DAG in Airflow UI
6. Test with manual trigger first
7. Then let it run automatically daily
"""
...
# Add dags directory to path so we can import the scraper
sys.path.insert(0, '/home/airflow/gcs/dags')

# Import the bulletproof scraper
from scraper_robust import BulletproofScraper
...
def scrape_dynamic_pages_robust(**context):
    """
    Task 2: Scrape dynamic pages using BulletproofScraper's 5-layer fallback.
    
    For each company:
      1. Try to find dynamic page URL (careers/blog/news)
      2. Use 5-layer scraping: live → snapshot → google cache → textise → job board
      3. Save to data/raw/<company>/<session_name>/
      4. Skip pages that don't exist
    """
    logger.info("="*70)
    logger.info("TASK 2: Smart Dynamic Page Scraping (Bulletproof)")
    logger.info("="*70)
    ...
    seed_path = '/home/airflow/data/forbes_ai50_seed.json'
    scraper = BulletproofScraper(seed_path)
    
    # Set session name for daily refresh
    session_name = f"{datetime.now().strftime('%Y-%m-%d')}_daily"
    scraper.session_name = session_name
    ...
    for i, company in enumerate(companies, 1):
        ...
        for page_type in DYNAMIC_PAGES:
            ...
            # Find page URL using scraper's smart finder
            page_url = scraper.safe_find_page_url(website, page_type)
            ...
            logger.info(f"    ✅ Found: {page_url}")
            
            # Page exists! Use bulletproof 5-layer scraping
            try:
                logger.info(f"    📥 Scraping {page_type} (with 5-layer fallback)...")
                
                html, text, metadata = scraper.safe_scrape_page(page_url, page_type)
                
                if not html or not text:
                    logger.error(f"    ❌ All fallbacks failed for {page_type}")
                    stats['pages_skipped'] += 1
                    continue
                ...
                (session_dir / f"{page_type}.html").write_text(html, encoding='utf-8')
                (session_dir / f"{page_type}.txt").write_text(text, encoding='utf-8')
                (session_dir / f"{page_type}_metadata.json").write_text(
                    json.dumps(metadata, indent=2),
                    encoding='utf-8'
                )
                
                company_result['pages_found'].append(page_type)
                company_result['sources_used'].append({
                    'page': page_type,
                ...
                if source in stats['fallback_usage']:
                    stats['fallback_usage'][source] += 1
                
                logger.info(f"    ✅ Saved {page_type} (source: {source})")
    ...
    logger.info(f"  News: {stats['page_type_counts']['news']}")
    logger.info(f"\nFallback usage:")
    for source, count in stats['fallback_usage'].items():
        if count > 0:
...
def generate_daily_report(**context):
    ...
    report = {
        'dag_id': 'ai50_daily_refresh_robust_dag',
        'execution_date': context['execution_date'].isoformat(),
        'status': 'SUCCESS',
        'total_companies': stats.get('total_companies', 0),
        'companies_with_updates': stats.get('companies_with_updates', 0),
        ...
        'pages_skipped_404': stats.get('pages_skipped', 0),
        'page_breakdown': stats.get('page_type_counts', {}),
        'fallback_usage': stats.get('fallback_usage', {}),
        'files_uploaded': files_uploaded,
        'scraper_version': 'bulletproof_v1.0',
        'completed_at': datetime.now().isoformat(),
    }
    ...
    for source, count in report['fallback_usage'].items():
        if count > 0:
            logger.info(f"     {source}: {count}")
...
with DAG(
    dag_id='ai50_daily_refresh_robust_dag',
    default_args=default_args,
    description='Lab 2: Daily refresh using bulletproof scraper with 5-layer fallback',
    schedule_interval='0 3 * * *',  # Daily at 3 AM UTC
    start_date=datetime(2025, 11, 3),
    catchup=False,
    tags=['orbit', 'lab2', 'daily', 'automated', 'bulletproof', 'robust'],
) as dag:
    ...
    # Task 2: Smart bulletproof scraping with 5-layer fallback
    task2_scrape = PythonOperator(
        task_id='scrape_dynamic_pages',
        python_callable=scrape_dynamic_pages_robust,
        execution_timeout=timedelta(hours=3),  # Longer timeout for fallbacks
    )
