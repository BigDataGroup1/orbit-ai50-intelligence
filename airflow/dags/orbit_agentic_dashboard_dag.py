...
# For Docker: /opt/airflow (mounted from project root)
# For GCP: /home/airflow/gcs/dags
project_root_docker = Path('/opt/airflow')
project_root_gcp = Path('/home/airflow/gcs/dags')

# Try Docker path first (local development)
if project_root_docker.exists():
    if str(project_root_docker) not in sys.path:
        sys.path.insert(0, str(project_root_docker))
# Fallback to GCP path
elif project_root_gcp.exists():
    if str(project_root_gcp) not in sys.path:
        sys.path.insert(0, str(project_root_gcp))
...
# Airflow paths (check both possible locations)
# For Docker: /opt/airflow/data (mounted from ./data)
# For GCP: /home/airflow/data
AIRFLOW_DATA_DIR = Path('/opt/airflow/data')  # Docker mount point
AIRFLOW_DATA_DIR_ALT = Path('/home/airflow/data')  # GCP location
AIRFLOW_DAGS_DIR = Path('/opt/airflow/dags')  # Docker mount point
...
def get_ai50_companies(**context):
    ...
    try:
        # Try to get from seed file (check multiple locations)
        import os
        seed_file = None
        
        # Try 1: AIRFLOW_DATA_DIR (Docker: /opt/airflow/data)
        seed_file = AIRFLOW_DATA_DIR / "forbes_ai50_seed.json"
        
        # Try 2: AIRFLOW_DATA_DIR_ALT (GCP: /home/airflow/data)
        if not seed_file.exists():
            seed_file = AIRFLOW_DATA_DIR_ALT / "forbes_ai50_seed.json"
        
        # Try 3: Local data directory (relative to DAG file)
        if not seed_file.exists():
            local_data_dir = Path(__file__).parent.parent.parent / "data"
            seed_file = local_data_dir / "forbes_ai50_seed.json"
        ...
        if not seed_file.exists():
            ...
            if enable_gcs_download:
                ...
                try:
                    ...
                    # Use AIRFLOW_DATA_DIR for downloaded file
                    seed_file = AIRFLOW_DATA_DIR / "forbes_ai50_seed.json"
                ...
                error_msg = f"""
                Seed file not found and GCS download is disabled.
                
                Checked locations:
                1. {AIRFLOW_DATA_DIR / "forbes_ai50_seed.json"}
                2. {Path(__file__).parent.parent.parent / "data" / "forbes_ai50_seed.json"}
                
                Please ensure seed file exists in one of these locations.
                For local development, place it in: data/forbes_ai50_seed.json
                """
                logger.error(error_msg)
                raise FileNotFoundError(f"Seed file not found. GCS download disabled.")
        else:
        ...
        for company in companies_data:
            ...
            if company_name:
                # Normalize to company_id format (lowercase, replace spaces with underscores)
...
def run_agentic_workflow_sync(company_id: str, company_name: str) -> dict:
    ...
    """
    # Add project root to path (where src/ folder is located)
    # For Docker: /opt/airflow (mounted from project root)
    # For GCP: /home/airflow/gcs/dags (parent of dags folder)
    project_root_docker = Path('/opt/airflow')
    project_root_gcp = Path('/home/airflow/gcs/dags').parent
    
    # Check which path exists and add to sys.path
    if (project_root_docker / 'src').exists():
        project_root = project_root_docker
        logger.info(f"Using Docker path: {project_root}")
    elif (project_root_gcp / 'src').exists():
        project_root = project_root_gcp
        logger.info(f"Using GCP path: {project_root}")
    else:
        error_msg = f"Could not find src/ folder. Checked:\n1. {project_root_docker / 'src'}\n2. {project_root_gcp / 'src'}"
        logger.error(error_msg)
    ...
    try:
        ...
        logger.info("Successfully imported run_workflow")
    except ImportError as e:
        logger.error(f"Failed to import run_workflow: {e}")
        logger.error(f"Current sys.path: {sys.path[:5]}")  # Show first 5 paths
        raise
...
def upload_dashboards_to_gcs(**context):
    ...
    if not enable_gcs_upload:
        ...
        logger.info(f"    - Dashboards: {AIRFLOW_DATA_DIR / 'dashboards' / 'workflow'}")
        logger.info(f"    - Traces: {AIRFLOW_DATA_DIR.parent / 'docs'}")
        logger.info(f"    - Reports: {AIRFLOW_DATA_DIR / 'airflow_reports'}")
    ...
    try:
        ...
        dashboard_dir = AIRFLOW_DATA_DIR / "dashboards" / "workflow"
        upload_count = 0
        ...
        trace_dir = AIRFLOW_DATA_DIR.parent / "docs"  # Traces are saved in docs/
        if trace_dir.exists():
...
def generate_agentic_report(**context):
    ...
    try:
        ...
        import os
        reports_dir = AIRFLOW_DATA_DIR / "airflow_reports"
...
with DAG(
    ...
    description='Orbit Agentic Dashboard: Run MCP + Agentic workflow for all AI 50 companies',
    schedule_interval='0 4 * * *',  # Daily at 4 AM UTC
    start_date=datetime(2025, 1, 1),
    catchup=False,
    tags=['orbit', 'agentic', 'mcp', 'workflow', 'dashboard'],
) as dag:
